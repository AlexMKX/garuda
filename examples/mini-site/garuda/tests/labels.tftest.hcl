...
run "ipt_server_label_router_id" {
  ...
  assert {
    condition     = output.ipt_server_label_router_id == var.ospf_router_ids.ipt_server
    error_message = "ipt_server label router_id must flow through from var.ospf_router_ids.ipt_server"
  }
}
