...
def reconcile_route(
    ...
    if len(desired) == 1:
        ipr.route(  # type: ignore[attr-defined]
            "replace", dst="default", gateway=desired[0], table=table
        )
    else:
        multipath = [{"gateway": ip} for ip in sorted(desired)]
        ipr.route(  # type: ignore[attr-defined]
            "replace", dst="default", multipath=multipath, table=table
        )
        ...
        while True:
            try:
                ...
                if providers:
                    ...
                    desired = sorted(resolve_nexthops(providers, neighbor_db))
                else:
                    desired = []
            ...
            except Exception as exc:  # noqa: BLE001 — daemon loop resilience
